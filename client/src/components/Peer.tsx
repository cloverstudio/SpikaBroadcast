...
export default ({ participant }: PeerInterface) => {
  const audioConsumer = participant.consumers.find(
    (consumer) => consumer.track.kind === "audio"
  );
  const videoConsumer = participant.consumers.find(
  ...
  return (
    <PeerView
      ...
      consumerSpatialLayers={
        videoScalabilityMode ? videoScalabilityMode.spatialLayers : null
      }
